...
@Slf4j
public class CreateAllEntitiesSpreadsheet {
	
	public void createAllEntitiesSpreedSheet(String EAFileName, String mcdFile, String exitFile){
		...
		try{
			...
			addCell(workbook, row, 8, "MCD");
			
			List<IModel> mdcModelList = MCDInteractionHelper.parseMCDExcelFile(mcdFile);
			Map<String, IModel> mdcMap = new HashMap<String, IModel>();
			for (IModel iModel : mdcModelList) {
				mdcMap.put(iModel.getName(), iModel);
			}
			...
			for (IModel iModel : allEAmodelList) {
				...
				IModel mdcModel = mdcMap.get(iModel.getName());
				String entityMCD =  "No";
				
				Map<String, IAttribute> mdcAttributeMap = null;
				
				if(mdcModel != null){
					entityMCD =  "Si";
					
					mdcAttributeMap = new HashMap<String, IAttribute>();
					for (IAttribute iAttribute : mdcModel.getAttributes()) {
