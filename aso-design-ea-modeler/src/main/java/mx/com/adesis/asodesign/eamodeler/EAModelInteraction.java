...
public class EAModelInteraction {
	...
	private void workOnNewEntity(Repository rep, IModel model, String packageGUID) throws Exception{
		...
		try
		{
			...
			{
				Element theElement = EAModelInteractionHelper.workOnNewElement(rep, thePackage, model.getName(), model.getDescription(), "Class", null);
	...
	private void workOnEntityAttributes(Repository rep, IAttribute modelAttribute, int elementID, Package thePackage) throws Exception{
		...
		try
		{
			...
			if(modelAttribute instanceof IEnumAttribute){
				...
				//Se crea enum
				Element theEnumElement = EAModelInteractionHelper.workOnNewElement(rep, thePackage, modelAttribute.getName(), modelAttribute.getDescription(), "Class", "enumeration");
			...
			if(modelAttribute instanceof IInterfaceAttribute){
				//Se crea la interfaz
				Element theInterfaceElement = EAModelInteractionHelper.workOnNewElement(rep, thePackage, modelAttribute.getName(), modelAttribute.getDescription(), "Interface", null);
				...
				EAModelInteractionHelper.workOnAttributeTagValue("json-param-resources", resourceList.toString(), newAttribute);
			}			
			...
			if(modelAttribute.getFormat() != null){
				EAModelInteractionHelper.workOnAttributeTagValue("json-param-format", modelAttribute.getFormat(), newAttribute);
			}
			if(modelAttribute.getPattern() != null){
				EAModelInteractionHelper.workOnAttributeTagValue("json-param-pattern", modelAttribute.getPattern(), newAttribute);
			}
			...
			if(modelAttribute instanceof IArrayAttribute){
				...
				if(iArrayAttribute.getMinItems() != null){
					EAModelInteractionHelper.workOnAttributeTagValue("json-param-minitems", String.valueOf(iArrayAttribute.getMinItems()), newAttribute);
				}
				if(iArrayAttribute.getUniqueItems() != null){
					EAModelInteractionHelper.workOnAttributeTagValue("json-param-uniqueitems", String.valueOf(iArrayAttribute.getUniqueItems()), newAttribute);
				}
			}
